...
function Index() {
  return (
    <>
      ...
      <section className="bg-surface py-20 md:py-28"><div className="section-shell"><SectionHeading eyebrow="Our Services" title="Solutions Designed for Business Growth" align="center" /><div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-4">{services.map((service, index) => { const icons = [Boxes, Building2, GraduationCap, Store]; const Icon = icons[index] ?? Boxes; const link = index === 3 ? "/services#rural-tech-store" : "/services"; return <article key={service.title} className="border border-border bg-card p-7 shadow-sm"><span className="grid size-12 place-items-center rounded-md bg-primary text-primary-foreground"><Icon /></span><h3 className="mt-8 font-display text-2xl font-bold">{service.title}</h3><p className="mt-4 min-h-24 text-sm leading-7 text-muted-foreground">{service.description}</p><Button asChild variant="link" className="mt-4 h-auto p-0"><Link to={link}>Learn More <ArrowRight /></Link></Button></article>})}</div><div className="mt-10 text-center"><Button asChild variant="outline" size="lg"><Link to="/services">View All Services <ArrowRight /></Link></Button></div></div></section>
      ...
      <section className="bg-hero py-20 text-hero-foreground md:py-24"><div className="section-shell"><div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[0.18em] text-accent-strong">Industries We Serve</p><h2 className="mt-3 font-display text-3xl font-bold md:text-4xl">Focused expertise where sales relationships matter.</h2></div><div className="mt-12 grid gap-8 md:grid-cols-3"><Industry icon={Boxes} title="SaaS">Helping technology companies reach and convert potential customers.</Industry><Industry icon={Building2} title="Logistics">Connecting logistics businesses with potential clients and business opportunities.</Industry><Industry icon={GraduationCap} title="Education">Supporting educational and training organizations in reaching learners and customers.</Industry></div></div></section>
      <CTASection />
    </>
  );
}
