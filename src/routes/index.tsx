  ...
  return (
    <>
      <section className="relative min-h-[680px] overflow-hidden bg-hero text-hero-foreground lg:min-h-[760px]">
        <img
          src={heroImage}
          alt="Business professionals collaborating on a sales growth strategy"
          width={1600}
          height={1104}
      ...
      <section className="py-20 md:py-28">
        <div className="section-shell">
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
            <IconCard icon={Target} title="Industry-Focused Expertise">
              Specialized sales solutions across SaaS, logistics, and education.
            </IconCard>
            <IconCard icon={Users} title="Customer-Centric Approach">
              We focus on understanding customer needs and creating meaningful connections.
            </IconCard>
            <IconCard icon={RouteIcon} title="Growth-Oriented Strategy">
              Our approach helps businesses expand their customer base and market presence.
            </IconCard>
            <IconCard icon={Handshake} title="Long-Term Partnerships">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {services.map((service, index) => {
              ...
              return (
                <article
                  key={service.title}
                  className="rise-in flex h-full flex-col border border-border bg-card p-7 shadow-sm transition-transform duration-300 hover:-translate-y-1"
                >
      ...
      <section className="bg-hero py-20 text-hero-foreground md:py-24">
        <div className="section-shell">
          ...
          <div className="mt-12 grid gap-8 md:grid-cols-2 lg:grid-cols-4">
            <Industry icon={Boxes} title="SaaS">
              Helping technology companies reach and convert potential customers.
            </Industry>
            <Industry icon={Building2} title="Logistics">
...
function Industry({ icon: Icon, title, children }: { icon: typeof Boxes; title: string; children: React.ReactNode }) {
  return (
    <article className="border-l-2 border-accent-strong pl-6">
      <Icon className="size-7 text-accent-strong" />
      <h3 className="mt-5 font-display text-xl font-bold">{title}</h3>
      <p className="mt-3 text-sm leading-7 text-hero-foreground/70">{children}</p>
    </article>
  );
}
